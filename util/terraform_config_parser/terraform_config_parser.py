...
def upload_configuration_to_s3(terraform_config: dict):
    ...
    simple_logger(f"Uploaded configuration.yaml to s3://{configuration_bucket_name}/{__get_key_name_from_config(terraform_config)}")
    terraform_config["config_bucket_name"] = configuration_bucket_name
    terraform_config["config_path_with_bucket"] = f"s3://{configuration_bucket_name}/{__get_key_name_from_config(terraform_config)}"
...
def replace_str_in_attribute(terraform_config: dict, source: str, target: str, replace_from: str, replace_to: str) -> dict:
    terraform_config[target] = terraform_config[source].replace(replace_from, replace_to)
    return terraform_config


def join_strings_in_attribute(terraform_config: dict, target: str, replace_from: list, join_char: str = "") -> dict:
    terraform_config[target] = join_char.join([str(terraform_config.get(x, '')) for x in replace_from])
    return terraform_config
...
if __name__ == "__main__":
    ...
    if not is_aws_profile_present():
        ...
        sys.exit(1)
    if terraform_config == {}:
        simple_logger("No output from terraform, are you in the deploy/infrastructure directory?")
        sys.exit(1)
    terraform_config = replace_str_in_attribute(terraform_config, "zone", "zone_safed", ".", "-")
