...
class AwsIntegrationHandler(BaseHandler):
    ...
    async def post(self):
        """
        POST the desired names for hub and spoke roles and get back the cloudformation URLs to configure all the things.
        """
        host = self.ctx.host
        data = tornado.escape.json_decode(self.request.body)
        hub_account_name = data.get("hub_account_name", "NoqCentralRole")
        spoke_account_name = data.get("spoke_account_name", "NoqSpokeRole")
        if not can_admin_all(self.user, self.groups, host):
            self.set_status(403)
            return
        external_id = config.get_host_specific_key("tenant_details.external_id", host)
        ...
        if hub_account_name:
            central_role_name = hub_account_name
        else:
            central_role_name = config.get(
                "_global_.integrations.aws.central_role_name", "NoqCentralRole"
            )
        if spoke_account_name:
            spoke_role_name = spoke_account_name
        else:
            spoke_role_name = config.get(
                "_global_.integrations.aws.spoke_role_name", "NoqSpokeRole"
            )
