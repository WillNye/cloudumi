...
async def generate_jwt_token(
    ...
    if not exp:
        exp = datetime.utcnow() + timedelta(
            hours=config.get(f"site_configs.{host}.jwt.expiration_hours", 1)
        )
    jwt_secret = config.get(f"site_configs.{host}.jwt_secret")
    if not jwt_secret:
    ...
    session = {
        ...
        "iat": iat,
        "exp": exp,
        config.get(f"site_configs.{host}.jwt.attributes.email", "email"): email,
        config.get(f"site_configs.{host}.jwt.attributes.groups", "groups"): groups,
        "host": host,
    }
...
async def validate_and_return_jwt_token(auth_cookie, host):
    ...
    try:
        ...
        email = decoded_jwt.get(
            config.get(f"site_configs.{host}.jwt.attributes.email", "email")
        )
        ...
        groups = decoded_jwt.get(
            config.get(f"site_configs.{host}.jwt.attributes.groups", "groups"), []
        )
        jwt_host = decoded_jwt.get(
            config.get(f"site_configs.{host}.jwt.attributes.host", "host"), ""
        )
