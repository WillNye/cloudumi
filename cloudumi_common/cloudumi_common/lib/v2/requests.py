...
from cloudumi_common.lib.templated_resources.requests import (
    generate_honeybee_request_from_change_model_array,
)
from cloudumi_common.lib.v2.aws_principals import get_role_details, get_user_details
...
async def populate_cross_account_resource_policy_for_change(
    ...
    sts_resource_policy_supported = config.get(
        f"site_configs.{host}.policies.sts_resource_policy_supported", True
    )
    supported_trust_policy_permissions = config.get(
        f"site_configs.{host}.policies.supported_trust_policy_permissions",
        [
