...
ecs-cli configure --cluster noq-staging-1 --default-launch-type FARGATE --config-name noq-staging-1 --region us-west-2
ecs-cli compose -f deploy/ecs/staging-1/docker-compose-ecs.yaml \
--cluster-config noq-staging-1 --ecs-params deploy/ecs/staging-1/ecs-params.yml -p noq-staging --task-role-arn arn:aws:iam::259868150464:role/NoqClusterRole1 \
