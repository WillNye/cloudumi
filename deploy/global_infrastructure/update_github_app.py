...
def generate_jwt(aws_profile, secret_key_arn):
    region = secret_key_arn.split(":")[3]
    ...
    response = secret_manager_client.get_secret_value(
        SecretId=secret_key_arn,
    )
    github_secrets = yaml.safe_load(response["SecretString"])
    pem_contents = github_secrets["_global_"]["secrets"]["github_app"]["private_key"]
    app_id = github_secrets["_global_"]["secrets"]["github_app"]["app_id"]
