...
output "aws_marketplace_subscription_queue_url" {
  description = "The SQS queue URL that is used to trigger customer registration using the NOQ CF templates"
  value       = join("", aws_sqs_queue.aws_marketplace_subscription_queue[*].url)
}
