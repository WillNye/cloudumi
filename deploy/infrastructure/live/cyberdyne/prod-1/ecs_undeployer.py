...
def get_image_tags(registry_name: str) -> List[str]:
    ...
    return [
        x for x in [y.get("imageTag") for y in image_info.get("imageIds", [])] if x is not None and "." in x
    ]
...
if __name__ == "__main__":
    ...
    if version is None:
        ...
        for registry in ELASTIC_REGISTRIES:
            ...
            resolved_versions[registry_name] = sorted(get_image_tags(registry_name), key=LooseVersion)

        ref_versions = list(sorted({x for n in resolved_versions.values() for x in resolved_versions[ref_registry_name] if any(k for k in n if k == x)}, key=LooseVersion))
