...
class TestFilterData(IsolatedAsyncioTestCase):
    ...
    async def test_and_operation(self):
        ...
        self.assertEqual(await filter_data(self.data, filter), expected_output)
    ...
    async def test_filter_no_matching_data(self):
        ...
        filter = {
            "tokens": [{"propertyKey": "city", "operator": "=", "value": "Los Angeles"}]
        }
        expected_output = []
        self.assertEqual(await filter_data(self.data, filter), expected_output)
    ...
    async def test_filter_with_invalid_operator(self):
        ...
        self.assertRaises(ValueError, filter_data, self.data, filter)
