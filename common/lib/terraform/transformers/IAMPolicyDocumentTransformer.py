    ...
    def transform_execution_policy(statements):
        ...
        for statement in statements:
            ...
            if "Action" in statement:
                ...
                actions = IAMPolicyDocumentTransformer.force_list(
                    statement.get("NotAction")
                )
                action_str = f"not_actions = {json.dumps(actions)}"
            condition_block = IAMPolicyDocumentTransformer.transform_conditions(
                statement
            )
            resources_list_str = json.dumps(
                IAMPolicyDocumentTransformer.force_list(statement.get("Resource"))
            ).replace("${", "$\\u0024{")
            ...
            for test, items in statement["Condition"].items():
                for variable, values in items.items():
                    values_str = json.dumps(
                        IAMPolicyDocumentTransformer.force_list(values)
                    ).replace("${", "$\\u0024{")
                    condition_block += f"""
    condition {{
      test     = "{test}"
