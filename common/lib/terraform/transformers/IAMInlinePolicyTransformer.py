...
class IAMInlinePolicyTransformer(BaseEntityTransformer):
    def __init__(self, entity_json: dict, principal_name: str, principal: Principal):
        policy_name = BaseEntityTransformer.safe_name_converter(
            entity_json["PolicyName"]
        )
        self.principal_name = principal_name
        self._safe_user_name = BaseEntityTransformer.safe_name_converter(
            self.principal_name
        )
...
"""
    ...
    def entities_to_import(self) -> list:
        return [
            {
                "identifier": f"aws_iam_{self._principal}_policy.{self.principal_name}_{self._safe_name}",
                "entity": f"{self.principal_name}:{self._entity_name}",
            }
        ]
