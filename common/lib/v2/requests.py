...
async def create_autogenerated_resource_policy_change_model(
    ...
    resource_policy = {**resource_policy}  # Don't want to modify original policy
    resource_type = resource_summary.resource_type
    supported_permissions = get_supported_resource_permissions(
        resource_summary.service, resource_type
    )
    # This way we can hit on intended actions and correct any case inconsistencies
    perm_map = {
        f"{resource_type}:{perm.lower()}": f"{resource_type}:{perm}"
        for perm in supported_permissions
    }
    for elem, statement in enumerate(resource_policy.get("Statement")):
        resource_actions = []
        for action in statement.get("Action", []):
            if resource_action := perm_map.get(action.lower()):
                resource_actions.append(resource_action)

        statement["Action"] = resource_actions
        resource_policy["Statement"][elem] = statement

    resource_policy_sha = sha256(
        json.dumps(resource_policy, escape_forward_slashes=False).encode()
    ).hexdigest()  # Recalculate in the event the policy changed
    return ResourcePolicyChangeModel(
        ...
        source_change_id=policy_change_id,
        id=str(uuid.uuid4()),
