...
ACCESS_UNDENIED_ROLE = "AccessUndeniedRole"
...
def lambda_handler(event, context):
    ...
    access_undenied_aws.cli.initialize_config_from_user_input(
        ...
        management_account_role_arn=(
            f"arn:aws:iam::{ACCOUNT}:role/{ACCESS_UNDENIED_ROLE}"
        ),
    ...
    scp_data = (
        boto3.Session()
        .client("s3")
        .get_object(Bucket=BUCKET_NAME, Key=BUCKET_KEY)["Body"]
        .read()
    )
    access_undenied_aws.organizations.initialize_organization_data(
        config=config, scp_file_content=scp_data
    )
