...
async def normalize_policy_actions_resources_to_list(policies: List[Any]) -> List[Any]:
    ...
    for policy in policies:
        ...
        if isinstance(policy.get("Action"), str):
...
async def generate_policy_from_permissions_string(permission: str) -> Dict[str, Any]:
    ...
    for item in permission_list:
        ...
        policy[item_key] = json.loads(item_value)
        if isinstance(policy[item_key], list):
            policy[item_key] = sorted(list(set(policy[item_key])))
    return policy
...
async def condense_statements(
    ...
    for statement in statements:
        for action_string in ["Action", "NotAction"]:
            for action in statement.get(action_string, []):
                action = action.lower()
                crafted_statement = copy.deepcopy(statement)
                crafted_statement[action_string] = [action]
                policy_string = await convert_policy_to_string(crafted_statement)
                derived_permissions_unique.add(policy_string)

    derived_permissions_inverted = []
    for policy_string in derived_permissions_unique:
        policy = await generate_policy_from_permissions_string(policy_string)
        derived_permissions_inverted.append(policy)

    deduplicated_statements = []
    for i in range(len(derived_permissions_inverted)):
        statement = derived_permissions_inverted[i]
        if await is_already_allowed_by_other_policy(
            statement,
            derived_permissions_inverted[:i] + derived_permissions_inverted[i + 1 :],
        ):
            continue
