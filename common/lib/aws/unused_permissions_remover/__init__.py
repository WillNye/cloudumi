...
from common.lib.aws.utils import (
    ...
    combine_all_policy_statements,
    condense_statements,
)


async def calculate_unused_policy_for_identity(host, arn, force_refresh=False):

    # Set tenant
    # Get all policies for a given role
    # TODO: Need to support a different flow for IAM users
    role = async_to_sync(fetch_iam_role)(
        account_id,
    ...
    aa_data = async_to_sync(retrieve_json_data_from_redis_or_s3)(
    ...
    individual_role_inline_policy_changes = calculate_policy_changes(
        role, used_services, policy_type="inline_policy"
    )
    print(individual_role_inline_policy_changes)

    individual_role_managed_policy_changes = calculate_policy_changes(
        role,
        used_services,
        policy_type="managed_policy",
        managed_policy_details=managed_policy_details,
    )
    print(individual_role_managed_policy_changes)

    before_combined = condense_statements(
    ...
    after_combined = condense_statements(
    ...
    bc = json.dumps(before_combined, indent=2)
    ac = json.dumps(after_combined, indent=2)
