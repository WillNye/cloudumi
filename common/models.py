...
#   timestamp: 2023-07-24T19:27:23+00:00
...
class PaginatedRequestQueryParams(BaseModel):
...
class IambicProvider(BaseModel):
    provider: str = Field(..., description="The name of the provider e.g. aws")


class IambicRequestTypeAheadFieldResponse(BaseModel):
    data: List[str] = Field(..., description="The type ahead field values")
    page: int = Field(..., description="The current page")
    next_page: Optional[int] = Field(None, description="The next page")
    page_size: Optional[int] = Field(None, description="The number of results per page")


class IambicProviderDefinition(BaseModel):
    id: str = Field(..., description="The provider definition id")
    name: str = Field(
        ..., description="A unique, human readable name for the provider definition"
    )
    provider: str = Field(..., description="The name of the provider e.g. aws")
    definition: Optional[Dict[str, Any]] = Field(
        None,
        description="An object containing template relevant information for the provider definition",
    )
...
class IambicTemplateChange(BaseModel):
    ...
    template_body: str = Field(..., description="The template's content")


class SelfServiceRequestChangeTypeField(BaseModel):
    field_key: str = Field(
        ..., description="The field key as defined in the change type field."
    )
    field_value: Union[str, int, bool, Dict[str, Any], List]


class SelfServiceRequestChangeType(BaseModel):
    change_type_id: str = Field(..., description="The change type id")
    provider_definition_ids: List[str] = Field(
        ..., description="The provider definition ids this change type is applicable to"
    )
    fields: Optional[List[SelfServiceRequestChangeTypeField]] = Field(
        None, description="The filled out form data for the change type"
    )


class SelfServiceRequestData(BaseModel):
    iambic_template_id: Optional[str] = Field(
        None, description="The IAMbic template ID the request applies to."
    )
    ...
    template_body: Optional[str] = Field(
        None,
        description="Used instead of changes + expires_at, this content will replace the current template body in its entirety.",
    )
    ...
    expires_at: Optional[str] = Field(
        None,
        description="When the granted access and/or permissions expire for the request.",
    )
    changes: Optional[List[SelfServiceRequestChangeType]] = None


class CreateOrPutExpressAccessRequestData(BaseModel):
    name: str = Field(..., description="The name of the express access request.")
    description: str = Field(
        ...,
        description="A brief description of what the express access request does or who it is for.",
    )
    change_type_id: str = Field(
        ...,
        description="The id of the change type this express access request applies to.",
    )
    iambic_template_id: str = Field(
        ...,
        description="The id of the iambic template this express access request applies to.",
    )
    suggest_to_all: bool = Field(
        ...,
        description='If all users should see this express access request as "Suggested"',
    )
    field_values: Optional[Dict[str, Any]] = Field(
        None,
        description="A dictionary used to set the values of the change type. The key is the field_key and the value is the field value. Note - field values do not need to be set.",
    )
    provider_definition_ids: Optional[List[Any]] = None


class PatchExpressAccessRequestData(BaseModel):
    name: Optional[str] = Field(
        None, description="The name of the express access request."
    )
    description: Optional[str] = Field(
        None,
        description="A brief description of what the express access request does or who it is for.",
    )
    suggest_to_all: Optional[bool] = Field(
        None,
        description='If all users should see this express access request as "Suggested"',
    )


class PatchChangeTypeData(BaseModel):
    name: Optional[str] = Field(
        None, description="The name of the express access request."
    )
    description: Optional[str] = Field(
...
class SelfServiceValidateRequestData(BaseModel):
    current_template_body: Optional[str] = Field(
        None, description="The current state of the IAMbic template as a yaml string."
    )
...
class SecretOIDCSettings(BaseModel):
    client_id: Optional[str] = Field(None, is_secret=True, title="Client Id")
    client_secret: Optional[str] = Field(None, is_secret=True, title="Client Secret")


class SecretAuthSettings(BaseModel):
    oidc: Optional[SecretOIDCSettings] = Field(None, is_secret=True, title="Oidc")
...
class GetUserByOIDCSettings(BaseModel):
    ...
    id_token_response_key: Optional[str] = Field(
        "id_token", title="Id Token Response Key"
    )
    access_token_response_key: Optional[str] = Field(
        "access_token", title="Access Token Response Key"
    )
    jwt_email_key: Optional[str] = Field("email", title="Jwt Email Key")
    jwt_groups_key: Optional[str] = Field("groups", title="Jwt Email Key")
    ...
    user_info_groups_key: Optional[str] = Field("groups", title="User Info Groups Key")


class JWTSAMLSettings(BaseModel):
    expiration_hours: Optional[int] = Field(1, title="Expiration Hours")
    email_key: Optional[str] = Field("email", title="Email Key")
    group_key: Optional[str] = Field("groups", title="Group Key")


class AttributesSAMLSettings(BaseModel):
    user: Optional[str] = Field("user", title="USER")
    groups: Optional[str] = Field("groups", title="GROUPS")
    email: Optional[str] = Field("email", title="EMAIL")


class BindingSAMLSettings(BaseModel):
    binding: str = Field(..., title="Binding")
    url: str = Field(..., title="Url")


class IdpSAMLSettings(BaseModel):
    entityId: str = Field(..., title="Entity Id")
    singleSignOnService: Optional[BindingSAMLSettings] = Field(
        None, title="Single Sign On Service"
    )
    singleLogoutService: Optional[BindingSAMLSettings] = Field(
        None, title="Single Sign On Service"
    )
    x509cert: str = Field(..., title="X509cert")


class SpSAMLSettings(BaseModel):
    Assertionconsumerservice: Optional[BindingSAMLSettings] = Field(
        None, title="Single Logout Service"
    )
    Singlelogoutservice: Optional[BindingSAMLSettings] = Field(
        None, title="Single Logout Service"
    )
    Entityid: Optional[str] = Field(None, title="Entity Id")


class GetUserBySAMLSettings(BaseModel):
    jwt: Optional[JWTSAMLSettings] = None
    attributes: Optional[AttributesSAMLSettings] = None
    idp: Optional[IdpSAMLSettings] = None
    sp: Optional[SpSAMLSettings] = None
    idp_metadata_url: Optional[str] = Field(None, title="Idp Metadata Url")


class ChallengeSamlSettings(BaseModel):
...
class OIDCSettingsDto(BaseModel):
    ...
    secrets: Optional[SecretAuthSettings] = None
    auth: AuthSettings


class SCIMSettings(BaseModel):
    enabled: Optional[bool] = Field(False, title="SCIM is Enabled")


class SCIMSettingsDto(BaseModel):
    scim: Optional[SCIMSettings] = None
...
class Secrets(BaseModel):
    ...
    jwt_secret: Optional[str] = Field(None, is_secret=True, title="Jwt Secret")
    auth: Optional[SecretAuthSettings] = Field(None, is_secret=True, title="Auth")
