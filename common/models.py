...
#   timestamp: 2022-08-10T16:18:11+00:00
...
class TearRoleChangeModel(ChangeModel):
    change_type: Optional[constr(regex=r"tear_can_assume_role")] = None
...
class PrincipalModelTearConfig(BaseModel):
...
class ExtendedAwsPrincipalModel(AwsPrincipalModel):
    ...
    elevated_access_config: Optional[PrincipalModelTearConfig] = None
...
class TearGroupConfig(BaseModel):
    name: str = Field(..., description="The name of the group")
    active_users_tag: Optional[str] = Field(
        None,
        description="Customer-specified or default tag used on roles to denote users that currently have TEAR access",
    )
    supported_groups_tag: Optional[str] = Field(
        None,
        description="Customer-specified or default tag used on roles to that a group can request TEAR access",
    )
...
class TearRoleConfig(BaseModel):
    ...
    supported_groups_tag: Optional[str] = Field(
        None,
        description="Customer-specified or default tag used on roles to denote groups that can request TEAR access",
    )
    requires_approval: Optional[bool] = Field(
        True,
        description="Optional value (defaults to true) to indicate whether Temporary Elevated Access requests require approval.",
...
class TearAccountConfig(BaseModel):
...
class CustomTearConfigs(BaseModel):
    group_configs: Optional[List[TearGroupConfig]] = []
    role_configs: Optional[List[TearRoleConfig]] = []
    account_configs: Optional[List[TearAccountConfig]] = []


class TearConfig(BaseModel):
    active_users_tag: Optional[str] = Field(
        None,
        description="Customer-specified or default tag used on roles to denote users that currently have TEAR access",
    )
    ...
    mfa: Optional[MfaSupport] = None
    custom_configs: Optional[CustomTearConfigs] = None
