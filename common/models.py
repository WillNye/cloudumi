...
#   timestamp: 2022-04-05T21:01:05+00:00
...
class HubAccount(BaseModel):
    name: Optional[str] = Field(
        None, description="Customer-specified or default hub role name (NoqCentralRole)"
    )
...
class SpokeAccount(BaseModel):
    name: Optional[str] = Field(
        None, description="Customer-specified or default spoke role name (NoqSpokeRole)"
    )
    account_name: Optional[str] = Field(None, description="Account name")
    account_id: constr(regex=r"^\d{12}$") = Field(..., description="AWS account id")
