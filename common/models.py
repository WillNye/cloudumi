...
#   timestamp: 2022-04-05T17:07:46+00:00
...
class HubAccount(BaseModel):
    uuid: UUID = Field(
        ...,
        description="A unique identifier for this hub account for frontend - backend interaction only",
    )
    name: str = Field(
...
class SpokeAccount(BaseModel):
    uuid: UUID = Field(
        ...,
        description="A unique identifier for this spoke account for frontend - backend interaction only",
    )
    name: str = Field(
...
class CognitoUser(BaseModel):
    ...
    TemporaryPassword: Optional[str] = Field(
        "T3mpP@ss!",
        description="A temporary password to set on the account after creation",
    )
