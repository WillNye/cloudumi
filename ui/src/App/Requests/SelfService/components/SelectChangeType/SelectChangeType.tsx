...
const SUGGESTED_CHANGES = [
  {
    header: 'Base Policy for Application',
    subtext: 'Provides access to essential services like S3, RDS, and EC2.'
  },
  {
    header: 'Manage Application-Specific Lambda',
    subtext:
      'Ability to create, update, and manage a Lambda function specific to the application.'
  },
  {
  ...
  {
    header: 'Create/Manage IAM Roles',
    subtext:
      'Allows creating, updating, and managing IAM roles and policies for fine-grained access control.'
  }
];

const SuggestedChangeTypes = () => {
  return (
    <div className={styles.container}>
      {SUGGESTED_CHANGES.map((change, index) => (
        <Card key={index} className={styles.card}>
          <h5 className={styles.header}>{change.header}</h5>
          <p className={styles.subtext}>{change.subtext}</p>
        </Card>
      ))}
    </div>
  );
