...
export const Auth: FC<PropsWithChildren> = ({ children }) => {
  ...
  const {
    ...
    error: ssoError,
    data: ssoData
  } = useQuery<WebResponse>(GET_SSO_AUTH_REDIRECT_QUERY);
  const [setupAPIAuth] = useMutation(AUTHENTICATE_NOQ_API_QUERY);

  // TODO (kayizzi) - Users must have a way to reset their password.
  ...
  useEffect(() => {
    if (ssoData) {
      ...
      // Check if user is already logged in
      console.log(ssoData);
      if (ssoData?.ssoAuthRedirect?.user) {
        setUser(ssoData.ssoAuthRedirect.user);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ssoData]);
  ...
  const getAuthenticatedUser = useCallback(async () => {
    ...
    // const { data, loading, error } = useQuery<WebResponse>(
    //   GET_SSO_AUTH_REDIRECT_QUERY
    // );
    // if (loading) {
    //   // Query is still being executed, so we await until it completes
    ...
    try {
      ...
      setIsGettingUser(false);
      throw new Error(`Error getting Authenticated user: ${message}`);
    }
  }, [setupAPIAuth]);
  ...
  const ssoLogin = useCallback(async () => {
    if (ssoData?.ssoAuthRedirect?.redirect_url) {
      window.location.href = ssoData.ssoAuthRedirect.redirect_url;
    } else {
      throw new Error(`Error logging in: No SSO Redirect URL found`);
    }
  }, [ssoData]);
  ...
  if (isGettingTenant || isGettingUser) {
